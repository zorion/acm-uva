...
def get_points(rule):
    start = (0, 0)
    for order in rule:
        points = parse_order(start, order)
        for p in points:
            yield p
        start = p  # The last point is the new start


def parse_order(point, order):
    command = order[0]
    times = int(order[1:])
    if command == 'D':
        i, j = 0, -1
        ...
        assert False, f'Weird order {order}'
...
def prob1(s):
    ...
    for i, rule in enumerate(rules):
        for p in get_points(rule):
            if p in data:
                if i == 1 and (min_cross is None or min_cross > manhattan(p)):
                    min_cross = manhattan(p)
            if i == 0:
                data.add(p)
    return min_cross


for k, vv in sample_in1.items():
    t1 = prob1(k)
    v, _ = vv
    assert t1 == v, f'Expected {v}, got {t1}.'
print('Samples part 1 ok')
my_in = """
...
def prob2(s):
    ...
    min_cross = None
    for i, rule in enumerate(rules):
        for step, p in enumerate(get_points(rule), 1):
            if p in data and i == 1:
                cur_val = data[p] + step
                if min_cross is None or min_cross > cur_val:
                    min_cross = cur_val
            if i == 0 and (p not in data):
                data[p] = step
    return min_cross
...
for k, vv in sample_in1.items():
    t2 = prob2(k)
    _, v = vv
    assert t2 == v, f'Expected {v}, got {t2}'
print('Samples part 2 ok')
my_res = prob2(my_in)
