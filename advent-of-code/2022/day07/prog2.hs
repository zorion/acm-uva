...
neededFree = 30000000
maxSpace :: Int
maxSpace = totalSpace -neededFree

prob1 :: String -> String
prob1 = show.sum.(filter (<100000)).walkTree.applyAllInstr.groupInstructions

prob2 :: String -> String
prob2 s = show(foldl min maxSpace folders)
    where aux = walkTree.applyAllInstr.groupInstructions $ s
          toRemove = (head aux) - maxSpace
          folders = filter (>= toRemove) aux
