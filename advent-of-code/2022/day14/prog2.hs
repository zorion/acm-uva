...
simulate (minRock, r) = if not (isFree minRock r positionInitial) then [] else r:res'
    where
...
dropSand n r p
    | n < y = r
    | isFree n r downPos = dropSand n r downPos
    | isFree n r dleftPos = dropSand n r dleftPos
    | isFree n r drightPos = dropSand n r drightPos
...
isFree :: Int -> Rocks -> Pos -> Bool
isFree n r p = and [(not $ Set.member p r), n > snd p]
...
getMinRock rs = (maxYNum + 2, rs)  -- Then minimum position is what have the maximum Y
